...
namespace ProjekWinform.Models
{
    public class PengirimanContext
    {
        public DataTable ReadPengiriman(string tanggal = "")
        {
            ...
            {
                string query = string.IsNullOrEmpty(tanggal)
                    ? "SELECT * FROM view_riwayat_pengiriman ORDER BY tanggal_pengiriman ASC"
                    : "SELECT * FROM view_riwayat_pengiriman WHERE CAST(tanggal_pengiriman AS TEXT) ILIKE @tanggal ORDER BY tanggal_pengiriman ASC";
        ...
        public void UbahStatus(int id_pengirim)
        {
            using (NpgsqlConnection conn = connectDB.GetConn())
            {
